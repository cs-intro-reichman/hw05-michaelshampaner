...
public class GameOfLife {
	...
	private static void test2(String fileName) {
		int[][] board = read(fileName);
		int numOfLive = count(board,2,3);
		System.out.println("number of live cells around him is " + numOfLive);
		int celllNewValue = cellValue(board, 2, 3);
		System.out.println("the cells new value with " + numOfLive + " live neighbors is " + celllNewValue);
	}
	...
	public static int[][] read(String fileName) {
		...
		for (int i = 1;i <= rows; i++) {
			...
			if (line == null || line.isEmpty()) {
				...
				for (int j = 1; j <= cols && j - 1 < line.length(); j++){
				// Convert characters to integers (assuming 'x' represents live and '.' represents dead)
